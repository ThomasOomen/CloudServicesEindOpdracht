...
describe('tests', function () {
	...
	beforeEach(function () {
		location1 = new LocationModel({ locationName: 'Eindhoven', lat: 80, long: 80, range: 20 });
		location2 = new LocationModel({ locationName: 'Den Bosch', lat: 80, long: 80, range: 20 });
		location1.save();
		location2.save();
	});
	...
	describe('location', function () {
		...
		describe('create new location', function () {
			...
			it('should validate lat', function (done) {
				location = new LocationModel({ locationName: 'Amsterdam', lat: 91, long: 80, range: 20 })
				const request = chai.request(app)
					.post('/location')
					.send(location)
					.end((err, res) => {
						...
						res.body.error.message.should.be.eql('location validation failed: lat: 91 is not a valid latitude!')
						done();
					});
			});
			it('should validate long', function (done) {
				location = new LocationModel({ locationName: 'Amsterdam', lat: 80, long: 91, range: 20 })
				chai.request(app)
					.post('/location')
					.send(location)
					.end((err, res) => {
						if (err) { return done(err); }
						res.should.have.status(400)
						res.body.error.message.should.be.eql('location validation failed: long: 91 is not a valid longitude!')
						done();
					});
			});
		})
